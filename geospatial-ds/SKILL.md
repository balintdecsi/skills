...
Credited as attribution, not endorsement.

Topics covered: Shapely geometries → GeoPandas vector data → rasterio/rioxarray rasters → OSMnx + geocoding + H3 spatial indexing.
...
Pin versions in `pyproject.toml` or `requirements.txt` for reproducible environments.
...
## Data Sources (free and good for projects)

Common open sources for geospatial projects:
...
| **GHSL — Global Human Settlement Layer (JRC)** | Population grids (`GHS_POP_*`), built-up surface, settlement model (e.g. `GHS_POP_E2030_GLOBE_R2023A_4326_30ss_V1_0.tif`) | <https://ghsl.jrc.ec.europa.eu/datasets.php> |
| **HydroSHEDS / HydroRIVERS** | Global rivers and watersheds | <https://www.hydrosheds.org/> |
...
More data-source detail: [reference/data_sources.md](reference/data_sources.md). CRS guidance: [reference/crs_cookbook.md](reference/crs_cookbook.md).
...
- File-format comparison: <https://milanjanosov.substack.com/p/geospatial-file-formats>
